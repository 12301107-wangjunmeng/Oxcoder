...
$("document").ready(
		function() {
			...
			$.ajax({
				...
				type : "post",
				dataType : "json",
				url : "coding-project-info.action",
				data : {
					"pid" : $("#input-reid").val(),
					"reid" : $("#input-reid").val(),
			...
			vm.$watch("projectOrder", function(newValue, oldValue) {
				if (vm.projectOrder >= 0 && vm.projectOrder <= 3) {
					$.ajax({
						async : true,
						type : "post",
						dataType : "json",
...
function compile(isSub) {
	...
	intervalCompile = setInterval(
			function() {
				...
				// 超时判断
				// if (vm.projectInfo.ptype==2||vm.projectInfo.ptype==3) {
				// if ((new Date()).getTime() - compileStartTime > 2000) {
				...
				$
						.post(
								"coding-compile.action",
								{
									"pid" : vm.projectInfo.pid,
									"code" : editor.getValue(),
									"reid" : $("#input-reid").val(),
									"requestOrder" : requestOrder,
									"isSub" : vm.projectRuntime.isSub,
									"order" : vm.projectOrder,
									"thinkTime" : thinkTime,
									"speed" : speed,
								...
								function(data) {
									...
									avalon.log(data);
									// 编译出错
									if (data.statusCode == COMPILE_INFO.error
											|| data.statusCode == COMPILE_INFO.success) {
									...
									} else if (data.statusCode == COMPILE_INFO.timeout) {
										console.log("编译超时");
										clearInterval(intervalCompile);
										vm.projectRuntime.timeoutCompile = true;
									}
